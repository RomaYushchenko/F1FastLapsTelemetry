...
- **Ініціалізація:** тестований об’єкт створюється в `@InjectMocks` на полі тестованого класу; не створюй його вручну в `@BeforeEach`. Mockito інжектить `@Mock` та `@Spy`. Для реальної залежності (наприклад маппер) використовуй `@Spy private SessionMapper sessionMapper = new SessionMapper();`.
...
| Mockito | Для класів з залежностями: `@ExtendWith(MockitoExtension.class)`, `@Mock` для залежностей, **`@InjectMocks`** для тестованого об’єкта; при потребі реальної залежності — `@Spy`. Не створювати тестований об'єкт вручну в `@BeforeEach`. |
