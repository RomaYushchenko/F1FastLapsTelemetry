...
**Чернетки планів покращень (draft):** [.github/draft/implementation-plans/](../../.github/draft/implementation-plans/) — 01 Logging, 02 Live page, 03 Session page (list), **04 Session Summary page** (Summary лідер, Lap pace overflow/медіана, Tyre wear %/compound, таблиця дельта/місце/стрілки, ERS), **05 UI notifications (toast)** — глобальні попап-повідомлення для помилок та фонових подій, **06 session type and track F1 25 alignment** — приведення маппінгу `m_sessionType` та (опційно) `m_trackId` до офіційної специфікації F1 25, **07 single place for session type and track display names** — одна точка на бекенді (telemetry-api-contracts + SessionMapper) для перетворення id → читабельні назви; фронт отримує готові `sessionType` та `trackDisplayName`. **08 Packet Event (ID 3)** — ingest і обробка пакета Event (DRSD, SCAR, RTMT тощо) з енумами. **09 Session packet enums** — заміна сирих int у PacketSessionData на reference enums (Weather, Formula, SafetyCarStatus тощо). **10 Lap Data enums** — PitStatus, Sector, DriverStatus, ResultStatus, PitLaneTimerActive; display names via LapDataMapper (WebSocket snapshot currentSectorDisplayName, REST/logs where lap data codes are available). See [10-lap-data-packet-enums-replacement.md](../../.github/draft/implementation-plans/10-lap-data-packet-enums-replacement.md). **11 Car Telemetry/Status/Damage enums** — DrsState, ErsDeployMode, FaultStatus, TractionControl, FuelMix, PitLimiterStatus; CarStatusMapper, ErsDeployMode in CarStatusProcessor and WsSnapshotMessage.ersDeployModeDisplayName. See [11-car-telemetry-status-damage-enums-replacement.md](../../.github/draft/implementation-plans/11-car-telemetry-status-damage-enums-replacement.md). **12 DRS/ERS fix** — DRS wing from Car Telemetry (m_drs), drsAllowed from Car Status; WsSnapshotMessage.drs + drsAllowed; ERS from Car Status (plan 11 enums). See [12-drs-ers-fix-enums-and-sources.md](../../.github/draft/implementation-plans/12-drs-ers-fix-enums-and-sources.md). **13 Session Summary speed/corner graph** — один графік на сторінці Session Summary: швидкість по дистанції кола та (опційно) метрики по поворотах (entry/apex/exit). Джерело: [speed-graf-deep-research-report.md](../../.github/draft/speed-graf-deep-research-report.md); план реалізації: [13-session-summary-speed-corner-graph.md](../../.github/draft/implementation-plans/13-session-summary-speed-corner-graph.md). Джерело enum-даних: [other-enume-types-deep-research-report.md](../../.github/draft/other-enume-types-deep-research-report.md).
