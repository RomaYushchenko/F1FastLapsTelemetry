...
**Новий UI (f1-telemetry-web-platform):** маршрути під префіксом `/app`. Головна — `/app`, список сесій — `/app/sessions`, деталі сесії — `/app/sessions/:id` (параметр `id` — string, session id з API). Усі запити до REST використовують `session.id` (string); пагінація списку — `GET /api/sessions?limit=&offset=`.

### 3.2 Навігація

- **Header / Nav:** посилання: "Live", "Sessions".
- **Live:** веде на `/` (dashboard); у новому UI — `/app/live`.
- **Sessions:** веде на `/sessions`; у новому UI — `/app/sessions`; з списку — клік по рядку/картці → `/sessions/{sessionUid}` (новий UI: `/app/sessions/{id}`).
- **Back:** з деталей сесії — кнопка "Back" до `/sessions` (новий UI: `/app/sessions`).
...
- **Speed** (speedKph), **RPM** (engineRpm), **Gear** (gear), **Throttle** (throttle), **Brake** (brake), **DRS** (drs = wing open from Car Telemetry; drsAllowed = zone from Car Status, plan 12), **ERS** (ersEnergyPercent, ersDeployActive, ersDeployModeDisplayName from plan 11), **Current lap / Sector** (currentLap, currentSector), **Delta to best** (deltaMs).
...
| **Session List** | `GET /api/sessions?limit=&offset=` (пагінація); `PATCH /api/sessions/{id}` (sessionDisplayName) | — |
| **Session Detail** | `GET /api/sessions/{uid}`, `GET .../laps?carIndex=`, `GET .../summary?carIndex=`, `GET .../pace`, `GET .../tyre-wear`, `GET .../laps/{lapNum}/trace`, `GET .../laps/{lapNum}/ers`, `GET .../laps/{lapNum}/speed-trace`, `GET .../laps/{lapNum}/corners` | — |

У **f1-telemetry-web-platform** (Block A) Session List і Session Detail використовують ці endpoints; session id у URL і в усіх викликах — string (UUID або session_uid з API).
...
### 7.4 Global notifications (toast)

- **Toast** — non-blocking popup messages in a fixed area (bottom-right), auto-dismiss after a few seconds, closable manually.
- **Variants:** error (REST 4xx/5xx, WebSocket errors), warning (connection lost, server messages), success (e.g. display name updated), info (e.g. live connected, session ended).
- **REST:** Failed requests (except 404) show one error toast from the API client; pages keep inline error + Retry for primary load.
- **WebSocket:** Connection errors, connect/disconnect/session-ended messages are shown as toasts so the user sees them even when not on the Live page.
- **When to use:** Primary load error → inline + optional toast; success of mutation → toast; background/WS error → toast; trace/ERS load error → inline + toast. Design tokens from § 3 (ui_ux_specification) are used for toast styling.
