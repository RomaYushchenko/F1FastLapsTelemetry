 ...
 * order is not guaranteed. We infer lap number from lap distance wrap-around (distance resets
 * when a new lap starts) so that telemetry for lap 2+ is stored under the correct lap.
 */
@Slf4j
@Component
...
public class CarTelemetryConsumer {
    ...
    public void consume(CarTelemetryEvent event, Acknowledgment acknowledgment) {
        try {
            ...
            log.info("Received car telemetry: sessionUID={}, frame={}, carIndex={}", sessionUid, frameId, event.getCarIndex());
    ...
    private Short resolveLapNumberForTrace(long sessionUid, short carIndex, SessionRuntimeState.CarSnapshot snapshot) {
        ...
        if (lapDistanceM != null) {
            ...
            // Detect new lap: distance reset (e.g. from ~5000m to ~0m)
